...
class DeleteCategoryTest extends TestCase
{
    ...
    public function testRunSuccess204(): void
    {
        ...
        $this->httpClientWrapper->expects($this->once())
            ->method('request')
            ->with('DELETE', 'private/categories/12', $headers)
            ->willReturn($this->response);

        DeleteCategory::run($this->client, 12, $headers);
    ...
    public function testRunSuccess404(): void
    {
        ...
        $this->httpClientWrapper->expects($this->once())
            ->method('request')
            ->with('DELETE', 'private/categories/100', [])
            ->willReturn($this->response);

        DeleteCategory::run($this->client, 100);
        $this->addToAssertionCount(1);
    }
    ...
    public function testRunWithTalerException(): void
    {
        ...
        DeleteCategory::run($this->client, 3);
    }
    ...
    public function testRunAsync(): void
    {
        ...
        $this->httpClientWrapper->expects($this->once())
            ->method('requestAsync')
            ->with('DELETE', 'private/categories/9', [])
            ->willReturn($promise);

        $result = DeleteCategory::runAsync($this->client, 9);
