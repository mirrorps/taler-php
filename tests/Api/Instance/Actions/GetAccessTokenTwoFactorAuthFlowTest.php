...
final class GetAccessTokenTwoFactorAuthFlowTest extends TestCase
{
    ...
    public function testGetAccessTokenTwoFactorFlowSuccess(): void
    {
        $instanceId = 'inst1';
        $request = new LoginTokenRequest('readonly');
        ...
        $streamChallenge->method('__toString')->willReturn(json_encode([
            'challenges' => [
                [
                    'challenge_id' => 'ch-access-success',
                    'tan_channel' => 'sms',
                    'tan_info' => '***1234',
                ],
            ],
                ...
                function (string $method, string $endpoint, array $headers = [], ?string $body = null) use (
                    ...
                    switch ($step) {
                        case 0:
                            ...
                            $self->assertSame("instances/{$instanceId}/private/token", $endpoint);
                        ...
                        case 1:
                            $self->assertSame('POST', $method);
                            $self->assertSame("instances/{$instanceId}/challenge/ch-access-success", $endpoint);
                            $self->assertIsString($body);
                            $step++;
                            return $responseRequest;
                        case 2:
                            $self->assertSame('POST', $method);
                            $self->assertSame("instances/{$instanceId}/challenge/ch-access-success/confirm", $endpoint);
                            $self->assertIsString($body);
                        ...
                        case 3:
                            ...
                            $expectedHeaders['Taler-Challenge-Ids'] = 'ch-access-success';
                        ...
                        default:
                            $self->fail('Unexpected additional HTTP request in GetAccessToken 2FA flow success test');
                    }
                }
            );

        // 1) Protected GetAccessToken -> ChallengeResponse
        $challengeResponse = GetAccessToken::run($this->instanceClient, $instanceId, $request);
        ...
        $this->assertSame('ch-access-success', $challengeId);
        ...
        // 4) Repeat GetAccessToken with Taler-Challenge-Ids header
        $result = GetAccessToken::run(
            $this->instanceClient,
            $instanceId,
            $request,
        ...
        $this->assertSame('Bearer abc', $result->access_token);
        $this->assertSame(4, $step, 'Expected exactly 4 HTTP calls in success 2FA flow');
    }

    public function testGetAccessTokenTwoFactorFlowFailsWithInvalidTan(): void
    {
        $instanceId = 'inst1';
        $request = new LoginTokenRequest('readonly');
            ...
            ->willReturnCallback(
                function (string $method, string $endpoint, array $headers = [], ?string $body = null) use (
                    ...
                    switch ($step) {
                        ...
                        case 1:
                            $self->assertSame('POST', $method);
                            $self->assertSame("instances/{$instanceId}/challenge/ch-access-fail", $endpoint);
                            $self->assertIsString($body);
                            $step++;
                            return $responseRequest;
                        case 2:
                            $self->assertSame('POST', $method);
                            $self->assertSame("instances/{$instanceId}/challenge/ch-access-fail/confirm", $endpoint);
                            $self->assertIsString($body);
                            $decoded = json_decode($body, true);
                        ...
                        default:
                            $self->fail('Unexpected additional HTTP request in GetAccessToken 2FA flow failure test');
                    }
                }
            );
        ...
        $this->assertCount(1, $challengeResponse->challenges);
        $challengeId = $challengeResponse->challenges[0]->challenge_id;
        $this->assertSame('ch-access-fail', $challengeId);
